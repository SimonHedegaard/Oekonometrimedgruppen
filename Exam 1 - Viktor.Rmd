...
$$
Equation71\\ : \text{Var}(u \mid \mathbf{x}) = \sigma^2 \exp\left(\delta_0 + \delta_1 x_1 + \delta_2 x_2 + \ldots + \delta_k x_k \right)
$$
...
Then we run a regression with the following form (which is ) to calculate the weights of our OLS model:
...
And then our FGLS can be calculated using the weights we just calculated. 
